...
- 增加 `skillcli update` 的 release/self-update 模式，用于从 release channel 更新 native 安装。
...
- 不改变 `skillcli update <project>` 的既有含义；它仍然表示更新一个源码 checkout。v0.1.1 起无 project 的 `skillcli update` 表示更新 `skillcli` 自身。
...
| S2 | 已安装用户执行 `skillcli update` | installer 从 release channel 获取 manifest、校验 artifact、切换 `current` |
...
| R4 | `skillcli update` 无 project 时必须复用 installer 语义，不和 `update <project>` 混淆 | CLI help / smoke command 检查 | 完成 |
...
2. **#2**: `skillcli update <project>` 继续表示源码 checkout 生命周期；release update 使用无 project 的 `skillcli update`。
...
2. `skillcli --version`、`doctor`、`update`、`uninstall --dry-run` 在 native 安装路径下可运行。
